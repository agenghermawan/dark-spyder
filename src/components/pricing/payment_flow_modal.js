...
export default function PaymentFlowModal({
                                             ...
                                             domainLimit = 1, // <= PASS THIS FROM PARENT FOR LIMIT
                                         }) {
    ...
    // direct show register form state
    const [showRegisterForm, setShowRegisterForm] = useState(false);

    // Fetch asset list if needed
    useEffect(() => {
    ...
    // Reset state on modal open/asset change or paymentData change
    useEffect(() => {
        setSelectedAsset(null);
        setSelectedBlockchain(null);
        setCreateError(null);
    ...
    // Show register form if forceRegisterDomain (from page) or paymentData.Status === 100
    useEffect(() => {
    ...
    const handleRegisterDomain = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/register-domain?invoiceId=${invoiceId}`, { // <-- tambahkan invoiceId ke URL
                method: "POST",
                credentials: "include",
                headers: {"Content-Type": "application/json"},
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-60 flex items-center justify-center px-4">
            <div className="relative w-full max-w-md bg-[#232339] rounded-2xl shadow-xl p-6 animate-fade-in">
                ...
                {/* LANGSUNG REGISTER DOMAIN JIKA forceRegisterDomain ATAU paymentStatus.Status === 100 */}
                {showRegisterForm ? (
