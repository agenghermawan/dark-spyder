...
export default function VulnerabilitiesPage() {
    ...
    const filteredVulnData = localSearch
        ? vulnData.filter(item =>
            // You can filter by name, template, severity, count, etc as needed
    ...
    if (authState === "loading" || (authState === "authenticated" && !domainLoaded)) {
        return (
        ...
        <div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
                    <div className="bg-[#232339] rounded-2xl shadow-xl p-6 max-w-md w-full relative">
                        <button
                            ...
                            aria-label="Close"
                        >×</button>
                        <h2 className="text-lg font-bold text-red-400 mb-4 text-center">Error</h2>
                        <div className="text-white text-center whitespace-pre-line">{errorModal.message}</div>
                    </div>
                </div>
            )}
            <div className="relative h-screen w-full">
                <section className="absolute inset-0 flex items-center justify-center px-4 sm:px-6 lg:px-8 text-white z-10">
                    <div className="max-w-3xl mx-auto text-center">
                        <h2 className="text-4xl font-bold mb-4">Vulnerability Assessment Results</h2>
                        ...
                        <div className="flex flex-row gap-2 max-w-xl mx-auto shadow-lg rounded-lg overflow-hidden w-full">
                            ...
                            >
                                {userDomains.map((dom, idx) =>
                                    <option key={dom + idx} value={dom}>{dom}</option>
                                )}
                            </select>
                            <button
            ...
            {(vulnData.length > 0 || showEmptyAlert) && (
                <section className="py-16 px-4 sm:px-6 lg:px-8" ref={resultsRef}>
                                        ...
                                        {filteredVulnData.map((item, idx) => (
                                            ...
                                            >
                                                <td className="py-4 px-6">
                                                    <span className={`px-4 py-1 rounded-full font-semibold ${severityColor[(item.severity || "critical").toLowerCase()] || severityColor["critical"]}`}>
                                                        {item.severity ? item.severity.charAt(0).toUpperCase() + item.severity.slice(1) : "Critical"}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="py-4 px-6">
                                                    ...
                                                    <span className="bg-[#23232b] px-2 py-1 rounded text-xs text-gray-300">{item.value}</span>
                                                </td>
                                                <td className="py-4 px-6">
                                                    <button
                            ...
                            <div className="mt-6 flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                                <div className="flex items-center gap-2">
                                    <p className="text-gray-500 text-sm">
                                        Showing {filteredVulnData.length} of {pagination.total} entries (Page {pagination.page})
                                    </p>
                                </div>
                    ...
                    <AssetDetailModal open={detailOpen} onClose={() => setDetailOpen(false)} item={detailItem} />
                </section>
            )}
        </div>
    );
